...
word.same_vowel_sounds #allpossible vowel breakdowns that have the same length as word.sylbl_sounds (list)
...
artist_obj_list: artists to vizualize (list of artist objects)\
...
exc_line = Flase: False means it will use the syllable breakdowns within the line the word lives in when optimizing matches. his is great for rappers that rhyme within the same line. rue will exclude that line when scoring matches (boolean)\
